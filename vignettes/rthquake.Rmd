...
The NOAA Earthquakes Data File can be downloaded from the NOAA site; it is not included in the package due to its size to help with installation times.  
...
After reading and cleaning the NOAA Earthquake Data Set, the information can be displayed in a timeline graph using two geoms functions designed for this task. These functions are developed using the ggplot package.
...
Subsetting is usually the best way to view this information.  One or more countries can be displayed for a particular time period.

To display the earthquakes in Mexico, USA and Jordan from January 1980 to January 2014:
...
To display the earthquakes in Mexico, USA and Jordan from January 1980 to January 2014 with labels:
...
To display a map of Earthquakes in Jordan after 1980 (more data):


```{r eval = FALSE}
filename<-system.file("data","earthquakes_data.txt.zip",package="capstone")
eq_location_clean(eq_clean_data(eq_data_read(filename))) %>%
dplyr::filter(COUNTRY == "JORDAN" & lubridate::year(DATE) >= 1980) %>%
